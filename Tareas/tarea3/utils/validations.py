...
import filetype
import io
from app import Transportes_disponibles
...
def validate_hincha(
    ...
    error = []
    if not validate_reg_com(region, comuna):  error.append("Region y Comuna") 
    if not validate_sports(sports):           error.append("Deportes")
    if not validate_transporte(transp):       error.append("Transporte")
    if not validate_comment(comment):         error.append("Comentarios adicionales")
    if not validate_name(name):               error.append("Nombre artesano(a)")
    if not validate_email(email):             error.append("Email contacto artesano(a) ")
    if not validate_phone(phone):             error.append("Número celular contacto")
    return error
...
def validate_sports(sports):
    for sport in sports:
        if not db.get_sport_id_by_name(sport):
            return False
    return True

def validate_transporte(transp):
    return True if transp in Transportes_disponibles else False

def validate_comment(comment):
    return 0 <= len(comment) <= 80
...
def validate_files(files):
    ...
    MAX_CONTENT_LENGHT = 1024*1024 ## 1MB
    files_size = 0
    ...
    for file in files:
        ...
        ## size of the file
        files_size += file.seek(0, 2)
        file.seek(0, 0)

    if files_size > MAX_CONTENT_LENGHT:
        return False
